...
2. **下載 Notion 文章中的圖片**：
   ...
   - 圖片檔名格式：`{原檔名}-{hash}.{副檔名}`
3. 根據 @blog/docs/BLOG_PRD.md 規格書整理文章格式
   - 將文章內容中的 Notion 圖片 URL 替換為本地路徑 `/assets/{新檔名}`
4. 生成符合 Strapi 格式的 JSON 檔案（儲存在 `blog/docs/{articleId}.json`）
...
- 相關檔案存在：`blog/docs/BLOG_PRD.md`、`create-article-script.js`、`blog/utils/article-graphql.js`

## 相關文件

- 技能說明：@blog/skills/notion-to-strapi/SKILL.md
- 使用教學：@blog/skills/notion-to-strapi/README.md
- 工作流程：@blog/skills/notion-to-strapi/workflow.md
