...
## 概述
notion 文章經過 @FORMAT_NOTE_TO_BLOG_PRD 規格書的整理後，可以透過 @article-graphql.js 的 createArticleFromNotion 函式，將 Notion 文章轉換為 Strapi 文章。
...
3. 使用 @FORMAT_NOTE_TO_BLOG_PRD 規格書的整理 Notion 文章為適合 Strapi 文章的資料格式，並且於 content-pipeline/docs 目錄下存成 {articleId}.json 檔案作為下一步的輸入資料。
...
```json
{
  ...
  "subTitle": "{subTitle}", // 按照 @FORMAT_NOTE_TO_BLOG_PRD 規格書的 subTitle 欄位生成的 subTitle
  "content": "{content}", // Notion 文章的原文，圖片路徑已替換為本地路徑 /assets/{新檔名}
  "description": "{description}", // 按照 @FORMAT_NOTE_TO_BLOG_PRD 規格書的 description 欄位生成的 description
  "articleId": "{articleId}", // 按照 @FORMAT_NOTE_TO_BLOG_PRD 規格書的 articleId 欄位生成的 articleId
  "publishedAt": "{publishedAt}" // 按照 @FORMAT_NOTE_TO_BLOG_PRD 規格書的 publishedAt 欄位生成的 publishedAt
}
```
